...
java -jar -Dwebdriver.ie.driver=.\PC\IEDriverServer.exe jar/selenium-server-standalone-3.0.1.jar -role node -port %port% -hub %hostaddress%/grid/register -browser browserName="internet explorer",version=%version%,platform=%platform%,maxInstances=%instances% -hubHost %hostaddress% -host %ipaddress%