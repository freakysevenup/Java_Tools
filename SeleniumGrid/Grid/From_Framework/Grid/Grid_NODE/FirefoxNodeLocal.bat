...
java -jar -Dwebdriver.gecko.driver=./Grid/Grid_NODE/PC/geckodriver.exe ./Grid/Grid_NODE/jar/selenium-server-standalone-3.0.1.jar -role node -port %port% -hub %hostaddress%/grid/register -browser browserName="firefox",version=%version%,platform=WINDOWS,maxInstances=%instances% -hubHost %hostaddress% -host %ipaddress%