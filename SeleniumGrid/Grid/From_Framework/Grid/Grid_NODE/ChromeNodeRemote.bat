...
java -jar -Dwebdriver.chrome.driver=.\PC\chromedriver.exe jar/selenium-server-standalone-3.0.1.jar -role node -port %port% -hub %hostaddress%/grid/register -browser browserName="chrome",version=%version%,platform=%platform%,maxInstances=%instances% -hubHost %hostaddress% -host %ipaddress% 
