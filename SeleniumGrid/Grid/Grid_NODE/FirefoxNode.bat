...
java -jar jar/selenium-server-standalone-3.0.1.jar -role node -port %port% -hub http://%ipaddress%:4444/grid/register -browser browserName="firefox",version=%version%,platform=WINDOWS,maxInstances=%instances% -hubHost http://%ipaddress%:4444 -host %ipaddress%